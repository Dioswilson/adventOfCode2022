...
int main() {
	...
	for (int i = 0; i < map.size; i++) {
		if (map.entries[i].height == 'a') {
			start = map.entries[i];
			pathLength = searchShortestPath(&map, &start, &end);
			if (pathLength < shortestPath || shortestPath == 0) {
...
int searchShortestPath(Map *map, MapEntry *start, MapEntry *end) {
	...
	while (current != NULL) {
		...
		for (Direction direction = NORTH; direction <= WEST; direction++) {
			...
			tempIndex=row * map->width + col;
			tempEntry = &map->entries[tempIndex];
