...
#define TOTAL_FILE_SYSTEM_SIZE 70000000
#define NEEDED_FREE_SPACE 30000000
...
int main(void) {
    ...
    int unusedSpace = 0;
    int sizeToDelete = INT_MAX; // Cound init in negatives and handle it
    ...
    unusedSpace = TOTAL_FILE_SYSTEM_SIZE - root->size;

    if (unusedSpace > NEEDED_FREE_SPACE) {
        printf("No need to delete files\n");
        return 0; // Looked good
    }
    ...
    while (tempDir != NULL) {
        ...
        if (tempDir->size >= NEEDED_FREE_SPACE - unusedSpace) {
            // printf("Size: %d\n", tempDir->size);
            if (sizeToDelete > tempDir->size) {
                sizeToDelete = tempDir->size;
            }
        }
